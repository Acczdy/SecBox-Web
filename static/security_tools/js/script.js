...
const proxy_listener_types = hoaxshell_listener_types;
const test_listener_types = hoaxshell_listener_types;
const clear_listener_types = hoaxshell_listener_types;
...
const rsg = {
    ...
    copyToClipboard: (text, label) => {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(function() {
                rsg._showToast(label || '命令')
            }).catch(function() {
                rsg.fallbackCopy(text, label);
            });
    ...
    _showToast: (label) => {
        var toast = $('#clipboard-toast');
        var body = toast.find('.toast-body');
        if (body.length) {
            body.text(label + '已复制');
        } else {
            toast.text(label + '已复制');
        }
        toast.toast('show');
    },
    ...
    getType: () => {
        const cmdName = rsg.getSelectedCommandName();
        if (rsg.commandType === 'HoaxShell' && hoaxshell_listener_types[cmdName]) {
            return hoaxshell_listener_types[cmdName];
        }
        if (rsg.commandType === 'Proxy' && proxy_listener_types[cmdName]) {
            return proxy_listener_types[cmdName];
        }
        if (rsg.commandType === 'Test' && test_listener_types[cmdName]) {
            return test_listener_types[cmdName];
        }
        if (rsg.commandType === 'Clear' && clear_listener_types[cmdName]) {
            return clear_listener_types[cmdName];
        }
        return 'cmd-curl';
    },
...
/*
    * Init - 等待数据加载完成后再初始化
    */
function initReverseShellApp() {
    if (window.rsgData && window.CommandType) {
        rsg.init();
        rsg.update();
        initReverseShellEvents();
    } else {
        setTimeout(initReverseShellApp, 50);
    }
}

function initReverseShellEvents() {
/*
...
//----------------------------------------------------------------------------------------------
            ...
            switch (option) {
              ...
              case 'python':
                ...
                taOutput.value = "python3 -c 'import base64;exec(base64.b64decode(\"" + btoa(taInput.value) + "\").decode())'";
                break;
              case 'perl':
